...
<?php
    class logins
    {
        ...
        public function getLogin($login_srno)
        {
            if($login_srno == null)
            {
            ...
            else
            {
                ...
                $sql = "select * from logins where login_srno = $this->login_srno;";
            }
        ...
        public function updateLogin($arr)
        {
            ...
            $this->password = $arr['password'];
            $this->user_srno = $arr['user_srno'];
            $sql = "update users set username = '$this->username', password = '$this->password', user_srno = '$this->user_srno' where login_srno = '$this->login_srno';";
            $stmt = $this->conn->prepare($sql);
            $stmt->execute();
            http_response_code(201);
        }
