...
namespace meetingattendanceControl;
use meetingattendances;
...
class meetingattendanceController
{
    ...
    function __construct($db, $requestMethod, $login_srno)
    {
        ...
        $this->meetingattendances = new meetingattendances($db);
    }
    ...
    public function processRequestForLogins()
        {
            switch($this->requestMethod)
            {
                case 'GET':
                    ...
                    {
                        $useritm = array('login_srno' => $res['login_srno'], 'username' => $res['username'], 'passowrd' => $res['passowrd'], 'user_srno' => $res['user_srno']);
